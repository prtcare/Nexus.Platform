...
**Status:** Active — the layer set is settled at twelve; most layers have no code yet and every
block below says so
**Owner:** Durai; each layer's own owner maintains its block
**Last updated:** 2026-08-21
**Layer:** cross-cutting
**Authoritative for:** the twelve layers — number, short name, long name, purpose, what each owns and
...
> belong to AI. Workflow belongs to AUTOMATION. Subscriptions belong to PRODUCT CORE. Milestones
> belong to DEVELOPER. Git and deployment belong to DELIVERY. Acceptance belongs to ASSURANCE.
> Runtime health belongs to OPERATIONS. Conversation belongs to EXPERIENCE. ERP belongs to
> PRODUCTS.**

Twelve nouns, twelve layers. If a new concept does not obviously slot next to one of those twelve
nouns, it is either misnamed or doing two jobs. That is worth stopping for, not working around.
...
| 07 | **DEVELOPER** | define, plan and build software |
| 08 | **DELIVERY** | source, build, environments, deployment, infrastructure |
| 09 | **ASSURANCE** | prove that requirements have actually been satisfied |
| 10 | **OPERATIONS** | run and observe deployed systems |
| 11 | **EXPERIENCE** | reusable human and system interaction |
| 12 | **PRODUCTS** | real business and customer problems |
...
2026-08-21. **Seven of the twelve layers have no project at all.**
...
| **Does NOT own** | The structured facts themselves. A milestone's completion percentage is DEVELOPER's; a product's owner is GOVERNANCE's. DATA owns the *document about* the fact and the ability to retrieve it |
...
| **Does NOT own** | What is being built (07) · how it ships (08) · how it runs (10) · document content (02 owns the trademark certificate PDF; GOVERNANCE owns the trademark status) · the business meaning of a product's features (12) |
| **Minimum before the gate** | **One record and nothing more.** A minimal `Product` — id, name, owner, classification, lifecycle state — because DEVELOPER's `ProductDevelopment` needs a `ProductId` to hang off |
...
DEVELOPER says what is *being built in it*. Two layers, two schemas, two lifecycles.
...
| **Does NOT own** | Product data · conversation storage (11) · documents (02) · provider credentials (01 holds those) · **any knowledge of what a `Workspace` or a `Milestone` is** |
...
| **Does NOT own** | The business meaning of what it runs — a workflow that approves a purchase order does not know what a purchase order is · reasoning, planning or model calls (04) · what work exists to be done (07) · deployment mechanics (08) · runtime telemetry (10) |
| **Minimum before the gate** | **Nothing, deliberately.** DEVELOPER V1a coordinates through explicit state transitions, not a workflow engine |
| **Deferred** | The entire layer. It becomes necessary when DEVELOPER V1b dispatches workers autonomously and needs somewhere deterministic for retry and escalation — P2, not P1 |
...
| **Does NOT own** | Identity (01) · product identity in the registry sense (03) · domain data (12) · **development structure below Subproject (07)** |
...
and `Project` can no longer belong to one product: DEVELOPER, a plain conversation and machine work
all need them, with completely different structure underneath.

---

### 07 — DEVELOPER
*define, plan and build software*

**Purpose.** Define, plan, build, test, review and coordinate software development, and be the
structured system of record for development state — replacing chat transcripts and markdown.

| | |
|---|---|
| **Repository** | `Nexus.Developer` — **does not exist yet** |
| **Schema** | `developer`. The first candidate to split to its own database at P3 — predicted, not decided (`DATABASE_ARCHITECTURE.md` §6) |
| **Projects (TARGET)** | `Nexus.Developer.Contracts`, `.Core`, `.Graph`, `.Orchestration`, `.Infrastructure`, `.Api`, `.Client` |
...
| **Does NOT own** | Product identity (03 — DEVELOPER references a `ProductId`) · `Workspace`/`Project`/`Subproject` (06 owns the trunk; DEVELOPER extends **downward** from Subproject) · repository and CI mechanics (08 produces a build, DEVELOPER interprets it) · whether a requirement was actually satisfied (09) · runtime health (10) · specification documents (02 — a `Milestone` links to a `Document` by ID) · conversation storage (11 — DEVELOPER supplies scope and context) |
...
**Developer consumes layers; it does not absorb them.** It does not implement chat — it registers
its scope hierarchy with PRODUCT CORE and implements EXPERIENCE's scope resolver. It does not run
CI — DELIVERY produces a build and DEVELOPER interprets whether it satisfies a work item. It does
not own documents. These three constraints are what stop the layer becoming a dumping ground, and
**scope creep here is the single highest risk in the plan**.
...
| **Today** | Does not exist. **New in v2.1** — inserting it renumbered OPERATIONS, EXPERIENCE and PRODUCTS |
...
| **Does NOT own** | What needs testing and which work item it belongs to (07) · executing build and automated-test pipelines (08) · runtime health (10) · the formal test report **document** (02 owns the document, ASSURANCE owns the result) · the `Requirement` itself (07 owns `Requirement`; ASSURANCE owns its `AcceptanceCriterion`) |
...
| **Today** | Does not exist. **Nothing is deployed, so nothing is operated.** The layer exists now to reserve the space and stop observability being retrofitted into twelve places later |
...
| **Does NOT own** | Anything durable about what was built or why (07) · how it was shipped (08) · whether a requirement was satisfied (09) |
...
DEVELOPER on milestones, a plain conversation on projects, machine work on operations — served by
one engine simultaneously, with zero shared types.
...
**A standalone Chat application, if released, is a Layer 12 product that consumes this engine.**
The engine is never called a Chat product.
...
its own safety architecture and human sign-off before any milestone may be written.
...
compliance state — owned across GOVERNANCE, DEVELOPER, DELIVERY and OPERATIONS, each marked derived
or manual. `PRODUCT_DEVELOPMENT_GUIDE.md` owns the procedure for standing one up.
...
|---|---|---|
| Chat | Conversation is universal; a chat *product* is a consumer | 11 EXPERIENCE (engine), 12 PRODUCTS (any app) |
| Frontend | Not a responsibility, a surface | 11 EXPERIENCE (shared), 12 PRODUCTS (product-specific) |
| Testing | Four different questions with four owners | 07 asks, 08 executes, 09 judges, 10 watches |
...
- `PRODUCT_DEVELOPMENT_GUIDE.md` — standing up a Layer 12 product.
- `AI_DEVELOPMENT_STANDARDS.md` — building on Layer 04 without breaking the seam.
- `ASSURANCE_STANDARDS.md` — Layer 09's methods, evidence and gates.
