...
Read C:\Personal\NexusAI\docs\NEXUS_ARCHITECTURE_V2.md section 1.3 and
C:\Personal\NexusAI\docs\ADR-014_AZURE_SQL_MIGRATION.md first.
...
### Schema cleanup rules — apply throughout Stage 1 and 2

The Dataverse solution export revealed 34 anomalies, catalogued in
`DATAVERSE_SCHEMA_REFERENCE.md`. **None of them are carried into SQL.** This migration is
the opportunity to delete them rather than immortalise them.

**Rule 1 — drop every `*01` column.** Artifacts of a column being created, renamed and
re-added; the unsuffixed name is real.

| Table | Drop | Keep |
|---|---|---|
| Project | `projectstatus01`, `projecttype01` | `projectstatus`, `projecttype` |
| Conversation | `conversationtype01` | `conversationtype` |
| ConversationLink | `linktype01` | `linktype` |
| ProjectMember | `projectmemberrole01` | `projectmemberrole` |
| Team | `teamstatus01` | `teamstatus` |

**Rule 2 — drop the two stray columns on WorkspaceMember.**
`workspacemembersrole` (note the extra `s`) is a typo duplicate of `workspacememberrole`,
which is the one with defined choice values. `workspacestatus` on `WorkspaceMember` is a
stray copy of `Workspace`'s own status and has no meaning on a membership row.

**Rule 3 — the C# enum is authoritative for every picklist.** Eleven picklists have no
choice values defined in the Dataverse export while the corresponding C# enum works today:
`AdrStatus`, `ArtifactType`, `BranchStatus`, `ConversationStatus`, `ConversationVisibility`,
`KnowledgeStatus`, `KnowledgeType`, `SessionStatus`, `WorkItemStatus`, `WorkItemType`,
`WorkspaceStatus`, `ProjectStatus`. Map the enum, ignore the Dataverse definition. Store as
`int` with an EF conversion; do not invent a lookup table.

This closes the standing debt item *"Dataverse registry may differ from live mappings —
reconcile logical names and choice values."*

**Rule 4 — one primary key per table.** Every Dataverse table carries both a GUID primary
key (`t_001_workspaceid`) and an `nvarchar(850)` business key (`workspaceid`). The second is
a Dataverse alternate-key artifact. Keep the GUID only — it matches the strongly typed IDs.
If a human-readable reference is genuinely wanted later, add it deliberately as a
`nvarchar(50)` with a unique index, not as an 850-character string.

**Rule 5 — two columns need a decision, not a rule.** `WorkItem.workitempriority` and
`Project.projecttype` exist in Dataverse with no C# counterpart. Either model them as domain
concepts or drop them. Do not create a column that nothing reads or writes.

**Rule 6 — drop the `du_` prefix and `T_nnn_` numbering.** Tables are `Workspace`,
`Project`, `Conversation`. The prefix was a Dataverse publisher requirement; the numbering
was a naming convention for a system that sorted alphabetically.
